...
<html>
    <head>
        ...
        printf("<title>$term - Sequence Search - EnchyBASE</title>\n");
        ?>

    </head>
    <body>
        <?php
        echo "<h1>Sequence information for <em>&lt;$term&gt;</em></h1>";
        ...
        if ($library == "all") {
            $query = "SELECT * FROM normal_ontology AS N WHERE N.sequence LIKE '%$term%' UNION SELECT * FROM pesticides_ontology AS N WHERE N.sequence LIKE '%$term%' UNION SELECT * FROM metals_ontology AS N WHERE N.sequence LIKE '%$term%'";
        } else {
            $query = "SELECT * FROM " . $library . "_ontology AS N WHERE N.sequence LIKE '%$term%'";
        }
        ...
        if ($result == 0) {
            print "<h2>$term was not found in the database!</h2><p>There are no sequence matches for the given <em>$term</em></p>";
