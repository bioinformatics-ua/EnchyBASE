...
<html>
    <head>
        ...
        printf("<title>$term - Ontology Search - EnchyBASE</title>\n");
        ?>
    ...
    <body>
        ...
        if (strpos($term, "GO:") === false) {
            $t_query = "SELECT * FROM ontology WHERE term LIKE '%$term%'";
            $t_dbres = mysql_query($t_query) or die('Error, term search failed');
            $t_result = mysql_num_rows($t_dbres);
            if ($t_result == 0) {
                print "<h2>$term was not found in the database!</h2><p>There are no ontology matches for the given <em>$term</em></p>";
                print "<a href=\"search.html\">Search again</a>";
                exit();
            } else {
                echo "<p>Search results for <em>$term</em></p>";
                while ($t_row = mysql_fetch_array($t_dbres)) {
                    $g = $t_row['id'];
                     
                    if ($library == "all") {
                        $query = "SELECT * FROM normal_ontology AS N WHERE N.gos LIKE '%$g%' UNION SELECT * FROM pesticides_ontology AS N WHERE N.gos LIKE '%$g%' UNION SELECT * FROM metals_ontology AS N WHERE N.gos LIKE '%$g%'";
                    } else {
                        $query = "SELECT * FROM " . $library . "_ontology AS N WHERE N.gos LIKE '%$g%'";
                    }
                    ...
                    if ($result == 0) {
                        ...
                        echo "<h1>Ontology information for <em>&lt;$g&gt;</em></h1>";
             ...
             echo "<h1>Ontology information for <em>&lt;$term&gt;</em></h1>";
            if ($library == "all") {
                $query = "SELECT * FROM normal_ontology AS N WHERE N.gos LIKE '%$term%' UNION SELECT * FROM pesticides_ontology AS N WHERE N.gos LIKE '%$term%' UNION SELECT * FROM metals_ontology AS N WHERE N.gos LIKE '%$term%'";
            } else {
                $query = "SELECT * FROM " . $library . "_ontology AS N WHERE N.gos LIKE '%$term%'";
            }
